...
fn token_idents(mut cursor: syn::buffer::Cursor<'_>, out: &mut BTreeSet<String>) { while !cursor.eof() { if let Some((ident, rest)) = cursor.ident() { out.insert(id(&ident)); cursor = rest; } else if let Some((inside, _, _, rest)) = cursor.any_group() { token_idents(inside, out); cursor = rest; } else if let Some((_, rest)) = cursor.token_tree() { cursor = rest; } else { break; } } }
fn expr_name(item: &Expr) -> String { match item { Expr::Path(value) => value.path.segments.iter().map(|part| id(&part.ident)).collect::<Vec<_>>().join("::"), Expr::Field(value) => format!("{}.{}", expr_name(&value.base), match &value.member { syn::Member::Named(name) => id(name), syn::Member::Unnamed(index) => index.index.to_string() }), Expr::Group(value) => expr_name(&value.expr), Expr::Paren(value) => expr_name(&value.expr), _ => "<expr>".into() } }
...
impl<'ast> Visit<'ast> for SourceVisitor<'_> {
    ...
    fn visit_expr_match(&mut self, item: &'ast syn::ExprMatch) { let parent = expr_name(&item.expr); if parent == "provider.provider_type" && self.file == "src/sdk/client/completions.rs" && self.owner == "execute_chat_request@LLMClient" { self.hit("provider-type-match"); } self.visit_expr(&item.expr); for (index, arm) in item.arms.iter().enumerate() { if matches!(arm.pat, syn::Pat::Wild(_)) { let owner = format!("wildcard-arm#{index}@match:{parent}@{}", self.owner); self.owned(owner, |this| visit::visit_arm(this, arm)); } else { self.visit_arm(arm); } } }
