...
fn codex_wire_round_trips_every_tier_one_field() {
    ...
    assert_eq!(CODEX_PROTOCOL_BASELINE, "6e5a2d6b8d148a5554fdceb6f399ca45bd1c78d9");
    for item in [
        ...
        let encoded = serde_json::to_value(serde_json::from_value::<ResponseInputItem>(item).unwrap()).unwrap();
...
fn codex_wire_distinguishes_tier_two_and_redacts_unknown_payload() {
    for value in tier_two_items() {
        assert!(matches!(serde_json::from_value::<ResponseInputItem>(value).unwrap(), ResponseInputItem::Unsupported(_)));
    }
    ...
    assert_eq!(serde_json::to_value(known).unwrap(), json!({"type":"local_shell_call","id":"i1","call_id":"c1","status":"completed"}));
    ...
    assert_eq!(serde_json::to_value(unknown).unwrap(), json!({"type":"future_item","id":"i2","namespace":"demo"}));
}
#[actix_web::test]
#[rustfmt::skip]
async fn codex_wire_http_rejects_before_provider_dispatch() {
    ...
    for tool in [
        ...
        assert!(!matches!(serde_json::from_value::<ResponseTool>(tool.clone()).unwrap(), ResponseTool::Unknown(_)));
        fixtures.push(json!({"model":"m","input":"x","tools":[tool]}));
    }
    for value in fixtures {
        let mut payload = codex_request(value);
        payload.store = Some(false);
        let req = actix_test::TestRequest::post().insert_header(("x-codex-upstream-counter", "1")).to_http_request();
        let response = super::create_response(state.clone(), req, web::Json(payload)).await.unwrap();
        assert_eq!(response.status(), StatusCode::BAD_REQUEST);
        let body: Value = serde_json::from_slice(&to_bytes(response.into_body()).await.unwrap()).unwrap();
        assert_eq!(body["error"]["type"], "invalid_request_error");
        assert_eq!(body["error"]["code"], "unsupported_codex_feature");
        assert!(body["error"]["message"].as_str().unwrap().contains("provider=unselected"));
        assert!(!body.to_string().contains("abcdefghijklmnop"));
    }
    assert_eq!(super::PROVIDER_DISPATCH_COUNT.load(std::sync::atomic::Ordering::Relaxed), 0);
}
