...
impl Router {
    ...
    ///
    /// Builds the new deployment and model-index maps in local DashMaps first,
    /// then replaces the live maps entry-by-entry so concurrent readers never
    /// observe an empty deployment window.
    pub fn set_model_list(&self, deployments: Vec<Deployment>) {
        ...
        // Atomically swap: remove stale keys, then upsert new ones.
        self.deployments
