...
- [ ] `SP831-T5` Owner: coordinator. Done when: router selection 或执行层先排除不可定价 deployment 候选，reject 策略下只有所有候选都不可定价才返回 4xx OpenAI 错误形状且不发往 provider；错误 code 明确为 `model_not_priced`. Verify: `cargo test spend --all-features`; `cargo test router --all-features`; 新增单测覆盖“未定价候选 + 可定价候选 → 选择可定价候选”、最终 reject 分支和 OpenAI `error.code == "model_not_priced"`。
- [ ] `SP831-T6` Owner: coordinator. Done when: `src/server/routes/ai/spend.rs:532-574`、`src/server/routes/ai/spend/pricing.rs:186`、`src/server/routes/ai/gemini/spend.rs`、`src/server/routes/ai/audio/budgeting.rs` 的 pricing-Err 分支改为共享 policy/结算辅助函数；allow 策略下非 0 fallback 在 provider 调用前创建 usage-scaled reservation / API-key hold，provider 返回后按 usage 缩放结算，有 usage 必结算（不 drop reservation）且 spend/usage 记录带 `unpriced=true`. Verify: `rg -n "unwrap_or\(0.0\)|pricing.*Err|calculate_loaded_usage_cost_for_provider" src/server/routes/ai/spend.rs src/server/routes/ai/spend/pricing.rs src/server/routes/ai/gemini/spend.rs src/server/routes/ai/audio/budgeting.rs`; 新增单测覆盖 allow/pre-call fallback reservation/usage-scaled fallback/settle/退款分支。
...
- [ ] `SP831-T9` Owner: verification owner. Done when: 复现测试证明默认配置下未定价模型请求被拒且预算不变、allow 配置下预留被结算且记录带标记、fallback 按 usage 缩放、可定价 deployment 优先于未定价候选、cache hit 不绕过 reject policy. Verify: `cargo test spend --all-features -- --nocapture`; `cargo test budget --all-features -- --nocapture`; `cargo test router --all-features -- --nocapture`; `cargo test cache --all-features -- --nocapture`（聚焦模块运行 <60s）。
