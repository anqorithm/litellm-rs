...
- [ ] `SP831-T4` Owner: coordinator. Done when: `unpriced_model_policy` / `unpriced_fallback_cost_per_1k_tokens` 配置模型、校验与默认值（reject）落地，fallback 只接受有限且 `>= 0.0` 的每 1k usage 单价，并写明 `pricing.allow_degraded` 只控制 pricing source 初始加载、请求期以 `unpriced_model_policy` 为准. Verify: `cargo test config --lib --all-features`; 新增 negative/NaN/inf fallback 拒绝测试；`rg -n "unpriced_model_policy|allow_degraded" src/config docs CHANGELOG.md`.
