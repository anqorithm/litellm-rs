...
7. response cache 命中不能绕过 unpriced policy：chat / embeddings 等缓存返回前也必须执行与路由候选一致的 usage-aware gate；
   至少一个健康候选可定价时才可返回缓存，全部候选不可定价时返回 `model_not_priced`。
8. 默认 `reject` gate 独立于预算是否启用；即使 provider/model/key 没有预算限制，也不能把未定价请求发往 provider。
...
- [ ] `spend.rs`、`spend/pricing.rs`、`gemini/spend.rs`、`audio/budgeting.rs`、image edit/variation proxy spend 等同模式路径行为一致。
...
- 预留成功但 settle 时才发现 unpriced（价格数据在请求期间被热更新移除）：不能用最大 hold 直接结算；
  必须按 provider 实际 usage 结合预检 quote/rates 或 usage-scaled fallback 结算并打标。
