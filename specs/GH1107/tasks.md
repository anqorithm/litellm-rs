...
- [ ] `SP1107-T017` Covers: implementation-route prerequisite. Owner: SpecRail workflow owner. Dependencies: T1 merged; no production Responses/Codex owner. Files: `checks/github_duplicate_evidence.py`, `checks/duplicate_work_gate.py`, `checks/route_gate.py`（仅在 route propagation 必要时）, `checks/test_duplicate_work_gate.py`, `schemas/duplicate_work_evidence.schema.json`, `specs/GH1107/tech.md`, `specs/GH1107/tasks.md`. Done when: collector 记录 lowercase 40-SHA `main` base 与完整 remote heads；required gate 在判定时以 `ls-remote` 重验 remote main/全部 matching branches，要求 local `origin/main` 与 evidence base 一致，并对每个 matching head 验证 local object 与 `merge-base --is-ancestor`；`maintainer_human` disposition 只作为 exact-SHA audit evidence，不单独授权 implement；stale/future、base/branch drift、object missing、non-ancestor、missing/duplicate/orphan/malformed/mismatch 全部 fail-closed，active duplicate/open PR 保持 blocked，且不删除历史分支. Verify: `PYTHONPATH=checks python3 -m unittest checks.test_duplicate_work_gate -v`；collector auth/fresh-base fixtures；stale/base/branch/all-matching/object/ancestor/schema negatives；required route propagation；`python3 checks/check_workflow.py --repo .`；`python3 checks/check_workflow.py --repo . --spec-dir specs/GH1107`；`git diff --check`.
