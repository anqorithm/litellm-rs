...
- [ ] `SP1107-T1` Covers: B-002, B-006, B-015, B-018. Owner: responses wire owner. Dependencies: spec approved + `ready_to_implement`. Files: `src/core/models/openai/responses_api.rs`, `src/core/types/codex.rs`, `src/core/types/mod.rs`, `src/server/routes/ai/openai_errors.rs`, `src/server/routes/ai/responses.rs`, `src/server/routes/ai/responses/codex_compat_tests.rs`, `src/server/routes/ai/responses/lifecycle.rs`, `src/server/routes/ai/responses/lifecycle_tests.rs`. Done when: Tier 1/Tier 2/unknown wire types 按 tech contract 可解析；`core/types/codex.rs` 在本 tranche 只建立独立 `wire` 子模块承载 wire DTO/serde/error helper，不含 `CodexTurn`、ledger 或 projection；Tier 1 的 `id`/`call_id`/`name`/`namespace`/payload 与 string/structured output form round-trip；unknown 只保留 metadata allowlist；非 message item、custom/Tier 2/unknown tool 和 `additional_tools` 在 call ledger 合并前统一返回 400 `unsupported_codex_feature`（message 含 feature/model/`provider=unselected`），且不进入 provider 执行路径。route/lifecycle 改动仅用于 fail-closed 与新增 optional 字段的编译适配，不得加入 call ledger、provider projection 或 previous-response call 恢复。fixtures 固定 Codex commit，正负例均 schema valid. Verify: `cargo test --locked codex_wire`，HTTP error envelope/upstream=0、missing/null/empty、unknown redaction、fixture source/count guard，fmt/check/strict Clippy。

- [ ] `SP1107-T017` Covers: implementation-route prerequisite. Owner: SpecRail workflow owner. Dependencies: T1 merged; no production Responses/Codex owner. Files: `checks/github_duplicate_evidence.py`, `checks/duplicate_work_gate.py`, `checks/route_gate.py`（仅在 route propagation 必要时）, `checks/schema_contract.py`, `checks/specrail_lib.py`, `checks/test_duplicate_work_gate.py`, `schemas/duplicate_work_evidence.schema.json`, `specs/GH1107/tech.md`, `specs/GH1107/tasks.md`. Done when: collector 记录 lowercase exactly-40-SHA `main` base 与完整 remote heads；required gate 先完成 closed schema/runtime validation，再以 `ls-remote` 重验 remote main/全部 matching branches，要求 local `origin/main` 与 evidence base 一致，并对每个 matching head 验证 local object 与 `git merge-base --is-ancestor <branch_head> <base_sha>`；`maintainer_human` 仅作为 exact-SHA audit provenance，不能单独授权未合并分支，只有 exact `retained_non_conflicting` + valid audit provenance + live merged-into-current-base proof 同时成立才 `allowed`；runtime schema published `pattern` 由 schema contract 校验，legacy instance validator 仍剥离 `pattern` 后由 gate `SHA_RE` 强制；stale/future、base/branch drift、object missing、not-merged、missing/duplicate/orphan/malformed/mismatch 全部结构化 fail-closed，active duplicate/open PR 保持 blocked，且不删除历史分支. Verify: `PYTHONPATH=checks python3 -m unittest checks.test_duplicate_work_gate -v`；collector auth/fresh-base fixtures；真实 ancestry 参数方向断言；stale/base/branch/all-matching/object/not-merged/name-only/schema negatives；allowed duplicate gate/required route；`python3 checks/check_workflow.py --repo .`；`python3 checks/check_workflow.py --repo . --spec-dir specs/GH1107`；`git diff --check`.

- [ ] `SP1107-T2` Covers: B-002, B-003, B-005, B-006, B-015, B-018, B-020. Owner: canonical turn owner. Dependencies: SP1107-T1 and SP1107-T017 merged; regenerate duplicate evidence against fresh remote heads and pass the required implement route (pre-merge evidence is invalid). Files: `src/core/types/codex.rs`, `src/core/types/mod.rs`, `src/server/routes/ai/responses/codex_compat.rs`, `src/server/routes/ai/responses/codex_compat_tests.rs`. Done when: 在 `core/types/codex.rs` 的独立 `domain` 子模块中完成 ordered CodexTurn、closed call kind、call ledger 和 execution requirements，且不把 domain 类型混入 `wire` 子模块；unknown/duplicate/missing/type mismatch 在 provider 前拒绝，且没有新 router 或 tool executor. Verify: `cargo test --locked codex_turn codex_call_ledger`，property/negative tests、source guard、fmt/check/Clippy。
