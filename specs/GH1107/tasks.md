...
- [ ] `SP1107-T017` Covers: implementation-route prerequisite. Owner: SpecRail workflow owner. Dependencies: T1 merged; no production Responses/Codex owner. Files: duplicate-work schema/collector/gate/tests and this packet only. Done when: any retained GH1107 contract branch has an explicit `maintainer_human` disposition bound to its exact fresh remote SHA; missing/duplicate/orphan/malformed/mismatched evidence remains fail-closed, active duplicate/open PR remains blocked, and no historical branch is deleted. Verify: duplicate-work unittest suite, fresh collector, duplicate gate and required implement route gate.
