...
- [ ] `SP837-T4` Owner: coordinator. Done when: registry conformance 守护测试合入——扫描 literal `impl LLMProvider` 与 `define_http_provider_with_hooks!` 等 macro-generated provider，并与「native enum/factory/dispatch + catalog-only 完成状态 + 豁免清单」求差集，非空失败；catalog 条目不得抵消仍存在的重复 native module. Verify: `cargo test core::providers::registry --lib --all-features`; 人为添加 literal impl、macro provider、catalog/native duplicate 三类孤儿 fixture 的负测试通过验证后移除。
...
- SP837-T5/T6 依赖 T3 批复与 T9 compatibility 决策；T5/T6 均依赖 T4 先合入（守护测试作为删除的安全网）。
