...
- [ ] `SP839-T3` Owner: coordinator. Done when: `http_facts()` canonical 映射模块合入（含当前动态 rate-limit headers、分离的 OpenAI vs canonical/internal code 词表、上游 OpenAI `ApiError` 字段透传策略），match 无 `_` 分支，全变体单测覆盖. Verify: `cargo test utils::error --lib --all-features`; `rg -n "fn http_facts" src/utils/error`.
- [ ] `SP839-T4` Owner: coordinator. Done when: 一致性遍历测试合入并以现状两张表为对照暴露全部漂移（首个红灯清单进 PR body）. Verify: 测试先红后绿的记录；`Cancelled` 漂移在清单中。
- [ ] `SP839-T5` Owner: coordinator. Done when: `openai_errors.rs` 与 `gateway_error/response.rs` 改为消费 `http_facts()`，两张手写 match 表删除；OpenAI `error.code` 与 canonical/internal `code`/`canonical_code` 响应值保持现状. Verify: `rg -c "StatusCode::" src/server/routes/ai/openai_errors.rs src/utils/error/gateway_error/response.rs` 显著收敛；一致性测试与 code 词表回归测试绿色。
- [ ] `SP839-T6` Owner: coordinator. Done when: OpenAI-compatible AI 路由统一显式渲染，`JsonConfig`/`QueryConfig`/`PathConfig` error_handler 与 auth/rate-limit 中间件拒绝接入同一 OpenAI renderer，错误 body `request_id` 与头一致；管理端 `/v1/*` 路由不被转换为 OpenAI 形状. Verify: 集成测试覆盖 extractor 三类失败、bad key、rate-limit、handler 失败形状一致；`rg -n "request_id: None" src/utils/error` 仅剩兜底注释。
- [ ] `SP839-T7` Owner: coordinator. Done when: `src/server/routes/mod.rs` 的管理端 errors helpers 携带语义状态码，`ApiResponse::to_http_response` 按 SP839-T2 决策保留/deprecated 或以 breaking change 明示删除，信封字段兼容测试通过. Verify: 若保留则编译测试覆盖 `to_http_response`; 若删除则 CHANGELOG/PR body 标明 public API break; `cargo test server::routes --all-features`。
