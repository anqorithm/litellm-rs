...
implementation, every production macro invocation that generates one, and the invoked macros' behavior owners.
These files are scoped only to the approved direct-cost disposition; unrelated provider behavior remains out of
scope. A truly new implementation or macro-invocation path discovered after this exact-head inventory must be
added by reviewed amendment before T3 writes it.
