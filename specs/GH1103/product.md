...
但公开 `core::cost` compatibility surface、legacy DTO、provider fallback 与新的
`pricing_service` 类型仍并存。当前没有独立 owner 定义这些兼容面的生命周期，维护者既无法安全删除，
也无法判断某个 fallback 是必要兼容还是仍在维持第二套 pricing authority。

## 目标

- 为 `core::cost` 的公开 symbol、production consumer、compatibility DTO、provider fallback
  和 test-only consumer 建立完整 inventory，并以已发布 `v0.5.0` tag/package API 为兼容基线。
...
- 不把 provider 内部仅用于展示、本地 catalog 或 protocol-specific 计算的类型机械纳入删除范围。
- 不在 spec approval、0.6 release evidence 与 version workflow gate 之前删除 public API。

## Behavior Invariants

1. 对同一 provider、model 与 usage，pricing route、budget reservation、spend settlement 与兼容 facade
   必须继续使用同一个 `PricingService` authority 结果。
2. 0.6.x 必须保持 `v0.5.0` tag/package 已发布 `core::cost` public symbol 的签名与既有兼容行为；只有 inventory
   中经维护者批准的 symbol 可以标记 deprecated，且必须给出替代路径。
3. 所有 user-visible provider fallback 的 lookup/calculation 必须在 `PricingService` authority 内执行；
   `core::cost` adapter 只能转换 canonical DTO/error。未归类 fallback 不得删除，也不得继续作为第二套 live
   pricing source，或把 unknown/incomplete pricing 静默变成 0 美元。
...
- [ ] inventory 由 `v0.5.0` tag/package public API 基线起步，并覆盖当前全部 `core::cost` re-export、production import、legacy DTO、provider fallback 和 test-only consumer；source guard 防止遗漏。
- [ ] 兼容矩阵对 public adapter 记录 `keep_adapter` 或 `deprecate_0_6_remove_0_7`，对 user-visible fallback 只允许 `migrate_authority` 或 `needs_decision`，并附 owner evidence。
...
- [ ] `PricingService` authority、spend parity、provider alias/fallback 与 unknown pricing fail-closed 回归全部通过。
