...
| D3Ca credential digest + conservative publication | `src/core/router/unified.rs`, `deployment.rs`, `gateway_config.rs`, `src/utils/auth/crypto/hmac.rs`, `src/utils/auth/crypto/tests.rs` | 5 files / ≤500；只为 proven credential 发布可选定长 digest，replacement 无 fresh provenance 时清 metadata，request path 每请求只 hash 一次后定长比较；`Refs #965`。 |
| D3Cb credential provenance normalization | `src/core/router/gateway_config.rs`, `deployment.rs`, `unified.rs` | 最多 3 files / ≤500；provider construction 与 metadata 共用同一 normalized effective credential，完整锁定既有 provider precedence；`Refs #965`。 |
