...
- 十二个 implementation slice 有严格依赖，必须按 policy -> HTTP client -> image-edit split -> image-router split -> completions split -> fixture consolidation -> config normalization -> runtime fixture preparation -> Gemini fallback fixture preparation -> Gateway/shared runtime -> shared extras -> native-route 串行合并，避免同一
