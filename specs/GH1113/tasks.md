...
- [ ] `SP1113-T2` Covers: B-001, B-002, B-004, B-005, B-007, B-009, B-010. Owner: exact pricing authority + typed discriminator owner. Dependencies: SP1113-T1 stable head. Files: `src/core/pricing_service/authority.rs`, `src/core/pricing_service/authority_tests.rs`, `src/core/providers/unified_provider_error.rs`, `src/core/providers/unified_provider_methods.rs` only；merged GH1112 Google registry is read-only. Done when: `PricingService` has one Developer/Vertex surface-aware exact canonical resolver over the loaded runtime source；Google path cannot enter fuzzy/longest/default/cross-surface fallback；token-priced records always validate both input/output price fields even for input-only/output-only/zero usage；missing/one-sided/invalid pricing returns a closed internal failure kind exhaustively mapped to a structured `ProviderError::PricingUnavailable`-equivalent variant；USD/token conversion has one owner；embedded compatibility initialization fails closed；non-Google resolver behavior unchanged. Verify: `cargo test --locked google_pricing_exact`；`cargo test --locked google_pricing_units`；`cargo test --locked pricing_service`；typed-variant/source/branch guard；fmt/check.
...
- [ ] `SP1113-T4` Covers: B-005, B-006, B-007, B-008, B-009. Owner: runtime policy/audit owner. Dependencies: SP1113-T3 stable head. Files: `src/server/routes/ai/spend/unpriced.rs`, `src/server/routes/ai/spend_runtime_pricing_tests.rs`, `src/server/routes/ai/spend_tests.rs`, `src/server/routes/ai/execution_retry_delay_tests.rs`. Done when: selected provider/canonical model/source matches reservation、settlement、spend and callback；default Reject stops before upstream/mutation/success effects；classifier matches only the structured pricing-unavailable variant and contains no Display/message-prefix parsing；an ordinary `InvalidRequest` carrying the legacy prefix cannot enter fallback；only that variant + explicit AllowUnpriced enters fallback；positive and explicit-zero fallback preserve provider/model/policy/outcome/cost in metrics、structured error log and key-scoped `UsageRecord::unpriced`；other errors never bypass；record failure emits error；retry/fallback re-resolves each candidate. Verify: `cargo test --locked google_pricing_runtime_parity`；`cargo test --locked allow_unpriced_audit_matrix`；`cargo test --locked execution_retry_delay`；message-prefix negative/source guard；counter/redaction fixtures；fmt/check.
...
- T2 owns pricing authority + structured error variant files and consumes the merged Google
  registry read-only；T3 exclusively owns provider/public helper files；T4 owns gateway policy
  classifier/tests and `spend/unpriced.rs`.
