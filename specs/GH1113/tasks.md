...
- [ ] `SP1113-T2` Covers: B-001, B-002, B-004, B-005, B-007, B-009, B-010. Owner: exact pricing authority + internal typed-discriminator owner. Dependencies: SP1113-T1 stable head. Files: `src/core/pricing_service/authority.rs`, `src/core/pricing_service/authority_tests.rs`, `src/core/pricing_service/mod.rs` only；merged GH1112 Google registry is read-only. Done when: `PricingService` has one Developer/Vertex surface-aware exact canonical resolver over the loaded runtime source；Google path cannot enter fuzzy/longest/default/cross-surface fallback；token-priced records always validate both input/output price fields even for input-only/output-only/zero usage；missing/one-sided/invalid pricing returns a closed crate-private `PricingFailureKind` available to request-time policy；public helper mapping uses only existing `ModelNotFound`/`Configuration` variants and the exhaustive public `ProviderError` enum is unchanged；USD/token conversion has one owner；embedded compatibility initialization fails closed；non-Google resolver behavior unchanged. Verify: `cargo test --locked google_pricing_exact`；`cargo test --locked google_pricing_units`；`cargo test --locked pricing_service`；internal-kind/no-public-variant/source/branch guard；fmt/check.
